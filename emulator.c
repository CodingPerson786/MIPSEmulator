...
int exec_bytecode() {
  ...
  //cgeck the 32 bit bytecode
  //start with add/addi
  //read opcode and compare with program segment
  //if opcode= 100000, $d=$s+$t (add)
  //if opcode==001000, $d=$s+i (addi)
 //get 10 program segments
 //find the program segment of the opcode
 //inside the program segment extract the first 16 bits
 //read register value
 //get regiister number, u get value
 //store val;ue in the array then add the parameter from the first 16 bits to the integer in the register (this is for addi)
  // print out the state of registers and memory at the end of execution

  //and 00000ff with opcode to get the last 3 bits
  //all bytecode is stored in array called text
  //check all the array aand extract bytecode
  //make ten if statemnets for the 10 opcodes
  //if opcode is 0 check first 6 bits if r type instruction

  while (text[TEXT_POS(pc)])
  {
	//r-type
	int instr=0;
	instr=instr|(0<<26);//opcode- shift opcode and sttore in location
	instr=instr|(17<<21);//rs
	instr=instr|(8<<16);//rt
	instr=instr|(17<<11);//rd
	instr=instr|(0<<6);//shamt
	instr=instr|(32);//funct

	//if(text[TEXT_POS(pc)])
	if((text[TEXT_POS(pc)]<<26)==0x00)//nop
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08) //add
	{
		
		//0x0f0000= $s1, 0x000f00=$s1, 0x00f000= $t1 AND all these and store in arr
	}

	if((text[TEXT_POS(pc)]<<26)==0x0C)//addi
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08)//andi (bitwise and immediate)
	{
	}
	
	if((text[TEXT_POS(pc)]<<26)==0x08)//lui (load upper immediate)
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08)//srl (shift left logical)
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08)//sll (logical shift left)
	{
	}	
	
	if((text[TEXT_POS(pc)]<<26)==0x08)//bne
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08)//lw (load word)
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08)//sw (store word)
	{
	}
	
	if((text[TEXT_POS(pc)]<<26)==0x08)//lhu (load halfword unsigned)
	{
	}

	if((text[TEXT_POS(pc)]<<26)==0x08)//sh (store halfword)
	{
	}
